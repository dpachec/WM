...
% paths
fs_template_path        = 'C:\Users\Neuropsychology\Desktop\ElectrodeLocalization_250520\FreesurferTemplate\';
dataoriginal_path       = 'C:\Users\Neuropsychology\Desktop\ElectrodeLocalization_250520\';
cd(dataoriginal_path);

addpath(genpath('C:\Users\Neuropsychology\Documents\MATLAB\spm12\'));
V_aparc                 = spm_vol(strcat(fs_template_path, 'aparc aseg-in-rawavg.nii'));
[Y_aparc, XYZ_aparc]    = spm_read_vols(V_aparc);
load(strcat(fs_template_path, 'fs_regionvalue2regionlabel.mat'));
...
clear fs_region_name
for chani = 1:size(MNI_coors, 1)
    % get index of this MNI coordinate in the Freesurfer image
    tmpidx              = find(XYZ_aparc(1, :) == MNI_coors(chani, 1) & XYZ_aparc(2, :) == MNI_coors(chani, 2) & XYZ_aparc(3, :) == MNI_coors(chani, 3));
                           

%     tmpidx              = find( ( XYZ_aparc(1, :) == MNI_coors(chani, 1)    | XYZ_aparc(1, :) == MNI_coors(chani, 1) +1 | XYZ_aparc(1, :) == MNI_coors(chani, 1) -1 ...
%                                 | XYZ_aparc(1, :) == MNI_coors(chani, 1) +2 | XYZ_aparc(1, :) == MNI_coors(chani, 1) -2 ...
%                                 | XYZ_aparc(1, :) == MNI_coors(chani, 1) +3 | XYZ_aparc(1, :) == MNI_coors(chani, 1) -3 ...
%                                 | XYZ_aparc(1, :) == MNI_coors(chani, 1) +4 | XYZ_aparc(1, :) == MNI_coors(chani, 1) -4) & ...
%                                 ( XYZ_aparc(2, :) == MNI_coors(chani, 2)    | XYZ_aparc(2, :) == MNI_coors(chani, 2) +1 | XYZ_aparc(2, :) == MNI_coors(chani, 2) -1 ...
%                                 | XYZ_aparc(2, :) == MNI_coors(chani, 2) +2 | XYZ_aparc(2, :) == MNI_coors(chani, 2) -2 ...
%                                 | XYZ_aparc(2, :) == MNI_coors(chani, 2) +3 | XYZ_aparc(2, :) == MNI_coors(chani, 2) -3 ...
%                                 | XYZ_aparc(2, :) == MNI_coors(chani, 2) +4 | XYZ_aparc(2, :) == MNI_coors(chani, 2) -4) & ...
%                                 ( XYZ_aparc(3, :) == MNI_coors(chani, 3)    | XYZ_aparc(3, :) == MNI_coors(chani, 3) +1 | XYZ_aparc(3, :) == MNI_coors(chani, 3) -1 ...
%                                 | XYZ_aparc(3, :) == MNI_coors(chani, 3) +2 | XYZ_aparc(3, :) == MNI_coors(chani, 3) -2 ...
%                                 | XYZ_aparc(3, :) == MNI_coors(chani, 3) +3 | XYZ_aparc(3, :) == MNI_coors(chani, 3) -3 ...
%                                 | XYZ_aparc(3, :) == MNI_coors(chani, 3) +4 | XYZ_aparc(3, :) == MNI_coors(chani, 3) -4));

    this_region_value   = Y_aparc(tmpidx);
    % get region label using freesurfer
    tmpidx              = find(cell2mat(fs_regionvalue2regionlabel(:, 1)) == this_region_value);
    fs_region_name{chani,:}      = fs_regionvalue2regionlabel{tmpidx, 2};
    EEG.chanlocs(chani).location = fs_regionvalue2regionlabel{tmpidx, 2};
end
coord = string(fs_region_name)
