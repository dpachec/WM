...
eval(  ['if exist(''new_' lower(contr2save{ci}) ''') & any(strcmp(contr2save, ''' contr2save{ci} ''')) ...' newline ...
           ...
           ' if nTrials > batch_bin ' newline ...
               ...
               ' for batchi = 1:nBatches ' newline ... 
                   'if strcmp(''' contr2save{ci}(end-1:end) ''', ''EE'')' newline ... 
                       ' if batchi == 1 ' newline ... 
                            ...
                            contr2save{ci} '{batchi}(:,1,:,:,:) = oneListPow (new_' lower(contr2save{ci}) '(t,1), :, :, 1:20); ...' newline ... 
                            contr2save{ci} '{batchi}(:,2,:,:,:) = oneListPow (new_' lower(contr2save{ci}) '(t,2), :, :, 1:20); ...' newline ... 
            ...
            'else' newline ...
                'if strcmp(''' contr2save{ci}(end-1:end) ''', ''EE'')' newline ... 
                    contr2save{ci} '{1}(:,1,:,:,:) = oneListPow (new_' lower(contr2save{ci}) '(:,1), :, :, 1:20); ...' newline ... 
                    contr2save{ci} '{1}(:,2,:,:,:) = oneListPow (new_' lower(contr2save{ci}) '(:,2), :, :, 1:20); ...' newline ... 
                'else' newline ... 
