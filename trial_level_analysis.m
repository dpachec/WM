...
paths = load_paths_WM(cfg.brainROI, cfg.net2load);
load([paths.results.DNNs f2sav '.mat']);
load([paths.results.clusters 'Alex_Enc_VVS.mat']);
...
for subji = 1:length(nnFit)
    ...
    nnF = nnFit{subji, 1}; 
    for triali = 1:size(nnF, 2)
        nnFT = squeeze(nnF(1, triali, :, 1:15)); 

        % % % % ALEXNET ENCODING
        fitTR(triali, :) = mean(nnFT(allClustInfo{1}.PixelIdxList{6}), 'all');
        %fitTR(triali, :) = mean(nnFT(allClustInfo{2}.PixelIdxList{4}), 'all');
        %fitTR(triali, :) = mean(nnFT(allClustInfo{3}.PixelIdxList{3}), 'all');
        %fitTR(triali, :) = mean(nnFT(allClustInfo{4}.PixelIdxList{6}), 'all');
        %fitTR(triali, :) = mean(nnFT(allClustInfo{5}.PixelIdxList{3}), 'all');
        %fitTR(triali, :) = mean(nnFT(allClustInfo{6}.PixelIdxList{2}), 'all');
        %fitTR(triali, :) = mean(nnFT(allClustInfo{7}.PixelIdxList{3}), 'all');
        %fitTR(triali, :) = mean(nnFT(allClustInfo{8}.PixelIdxList{4}), 'all');
    end

    ids = nnFit{subji, 2}; 
    ids = cellfun(@(x) strsplit(string(x)), ids, 'UniformOutput', false);
    
    % % % % item or category level 9 or 8
    ids = double(string(cellfun(@(x) x(9), ids, 'UniformOutput', false)));
...
h = bar (mean_S);hold on;
set(h,'FaceColor', 'none', 'lineWidth', 3);
set(gca,'XTick',[1 2],'XTickLabel',{'', ''}, 'FontSize', 30, 'linew',2, 'xlim', [0 3], 'ylim', [-.015 .035] );
plot(get(gca,'xlim'), [0 0],'k','lineWidth', 3);
...
exportgraphics(gcf, 'myPNG.png', 'Resolution',100)
